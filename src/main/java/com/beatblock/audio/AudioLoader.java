...
/**
 * 负责加载音频资源（WAV 路径），解码后分析波形与频段并写入 TimelineModel，同时更新 MusicPlayer 时长。
 */
public class AudioLoader {
	...
	/**
	 * 加载 WAV 文件：解码 → 波形 + 频段分析 → 写入 TimelineModel，并设置 MusicPlayer 时长。
	 *
	...
	public boolean load(String pathOrId) {
		...
		TimelineModel timeline = BeatBlock.timelineModel;
		if (timeline != null) {
	...
	public void loadFromDecoded(DecodedAudio audio) {
		...
		if (BeatBlock.timelineModel != null) {
			AudioAnalyzer.analyzeAndFillTimeline(audio, BeatBlock.timelineModel);
		}
		if (BeatBlock.musicPlayer != null) {
			BeatBlock.musicPlayer.setDurationSeconds(audio.getDurationSeconds());
			BeatBlock.musicPlayer.setCurrentTimeSeconds(0);
	...
	/**
	 * 卸载资源（仅清除加载标记，不清空 TimelineModel）。
	 */
