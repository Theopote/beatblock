...
| **pip 依赖** | `src/main/resources/beatblock/analyzer/requirements.txt`（librosa、numpy、scipy 等） |
...
可选 **Demucs 分轨**（体积大，含 PyTorch）：

```bash
pip install -r src/main/resources/beatblock/analyzer/requirements-demucs.txt
```

模组可在「音频解析」面板中检测 Python / ffmpeg 环境，并尝试自动安装分析依赖。
...
- 分析脚本：`src/main/resources/beatblock/analyzer/analyze.py`
...
本项目采用 [CC0-1.0](LICENSE) 许可证（见 `fabric.mod.json` 与仓库 `LICENSE` 文件）。
